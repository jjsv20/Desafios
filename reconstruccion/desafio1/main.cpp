...
int main()
{
    ...
    for (int i = 0; i < width * height * 3; i += 3) {
        unsigned char valor = i % 256;  // Mantiene el valor entre 0-255
        pixelData[i] = valor;     // Canal rojo
        pixelData[i + 1] = valor; // Canal verde
        pixelData[i + 2] = valor; // Canal azul
 ...
 */
    ...
    // Leer y almacenar los valores RGB uno por uno en el arreglo dinámico//******MODIFICAR POSIBLE SOLUCION******
    for (int i = 0; i < n_pixels * 3; i += 3) {
        archivo >> r >> g >> b;
        RGB[i] = r;
        RGB[i + 1] = g;
